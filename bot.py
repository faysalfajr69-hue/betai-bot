...
def search_team(team_name):
    """İsme göre takım arar, en iyi eşleşen takımı döner (id, name, country)."""
    data = _api_get("teams", {"search": team_name}, FIXTURES_TTL)
    resp = data.get("response", [])
    return resp[0]["team"] if resp else None


def get_team_players_season_stats(team_id, season=None):
    """Bir takımın o sezonki tüm oyuncularının istatistiklerini (şut dahil) çeker.
    Sezonu bilmiyorsak önce bu yılı, olmazsa bir önceki yılı dener (Ağustos-Mayıs
    ligleri için sezon başlangıç yılıyla anılır, ör. 2025-26 sezonu -> season=2025)."""
    candidate_seasons = [season] if season else [date.today().year, date.today().year - 1]
    for s in candidate_seasons:
        players = []
        page = 1
        while True:
            data = _api_get("players", {"team": team_id, "season": s, "page": page}, MATCH_DATA_TTL)
            resp = data.get("response", [])
            players.extend(resp)
            paging = data.get("paging", {})
            if page >= paging.get("total", 1) or page >= 3:  # kota koruması: en fazla 3 sayfa
                break
            page += 1
        if players:
            return players, s
    return [], candidate_seasons[0]


def summarize_team_shot_stats(players, top_n=5):
    """Oyuncu listesinden takım geneli ve en çok şut çeken oyuncuları (gerçek veriden) hesaplar."""
    rows = []
    team_shots_total = 0
    team_shots_on = 0
    team_games = 0
    for p in players:
        ...
        for stat in p.get("statistics", []):
            games = stat.get("games", {}).get("appearences") or 0
            shots = stat.get("shots", {}) or {}
            total_shots = shots.get("total") or 0
            on_shots = shots.get("on") or 0
            if games and games > 0:
                rows.append({
                    ...
                    "shots_on": on_shots,
                    "avg_shots": round(total_shots / games, 2),
                    "avg_on": round(on_shots / games, 2),
                })
                team_shots_total += total_shots
                team_shots_on += on_shots
                team_games = max(team_games, games)

    rows.sort(key=lambda r: r["shots_total"], reverse=True)
    team_avg_shots = round(team_shots_total / team_games, 2) if team_games else 0
    team_avg_on = round(team_shots_on / team_games, 2) if team_games else 0
    return {
        "team_avg_shots_per_game": team_avg_shots,
        "team_avg_shots_on_per_game": team_avg_on,
        "top_players": rows[:top_n],
    }


def get_team_multi_season_stats(team_id, league_id, num_years=5):
    """Bir takımın son N sezonluk istatistiklerini (galibiyet/gol/form) çeker.
    Sadece kullanıcı gerçekten bir maça baktığında çağrılır, kota bu şekilde korunur."""
    current_season = date.today().year
    results = []
    for offset in range(num_years):
        season = current_season - offset
        stats = get_team_statistics(team_id, league_id, season)
        if not stats or not stats.get("fixtures"):
            continue
...
AI_SYSTEM_PROMPT = (
    ...
    "6) ÇOK ÖNEMLİ: Sana verilmeyen hiçbir rakamı/istatistiği UYDURMA. Örneğin oyuncu şut sayısı, gol sayısı, "
    "yüzde gibi somut bir rakam istendiğinde ve bu rakam sana verilen veri içinde yoksa, tahmini bir sayı "
    "söylemek yerine açıkça 'bu veri elimde yok' de ve varsa yakın/ilgili gerçek verilerle sınırlı kal."
)
...
def main_menu():
    rows = [
        ...
        [InlineKeyboardButton("🎯 Takım Oyuncu Şut İstatistikleri", callback_data="menu:teamstats")],
...
def match_detail_menu(fixture_id):
    rows = [
        ...
        [InlineKeyboardButton("📅 5 Yıllık Geçmiş", callback_data=f"match:{fixture_id}:history")],
...
async def prompt_team_stats(update, context):
    query = update.callback_query
    context.user_data["mode"] = "awaiting_team_stats"
    await query.edit_message_text(
        "🎯 Hangi takımın oyuncu şut istatistiklerini görmek istersin? Takım adını yaz (örn: `Galatasaray`).",
        reply_markup=back_home_menu(),
        parse_mode="Markdown",
    )


async def handle_team_stats_text(update, context):
    team_name = update.message.text.strip()
    context.user_data.pop("mode", None)
    await update.message.reply_text(f"⏳ {team_name} aranıyor ve oyuncu istatistikleri çekiliyor...")

    team = search_team(team_name)
    if not team:
        await update.message.reply_text(f"'{team_name}' adında bir takım bulamadım. Yazımı kontrol edip tekrar dener misin?")
        return

    players, season = get_team_players_season_stats(team["id"])
    if not players:
        await update.message.reply_text(
            f"*{team['name']}* için oyuncu istatistiği verisi bulunamadı (API-Football bu takım/sezon için "
            f"veri sağlamıyor olabilir).", parse_mode="Markdown"
        )
        return

    summary = summarize_team_shot_stats(players)
    lines = [
        f"🎯 *{team['name']}* — {season}/{season+1} Sezonu Oyuncu Şut İstatistikleri\n",
        f"📊 Takım geneli maç başı ortalama: {summary['team_avg_shots_per_game']} şut, "
        f"{summary['team_avg_shots_on_per_game']} isabetli şut\n",
        "🏅 *En çok şut çeken 5 oyuncu:*",
    ]
    for i, p in enumerate(summary["top_players"], 1):
        lines.append(
            f"{i}. {p['name']} — {p['games']} maç, toplam {p['shots_total']} şut ({p['shots_on']} isabetli) "
            f"→ maç başı {p['avg_shots']} şut ({p['avg_on']} isabetli)"
        )
    if not summary["top_players"]:
        lines.append("Veri bulunamadı.")

    lines.append("\n_Bu rakamlar API-Football'dan doğrudan çekilen gerçek verilerdir, yapay zeka tahmini değildir._")
...
def _fixture_context_text(fixture_id, chat_data):
    ...
    home_hist = get_team_multi_season_stats(home_id, league_id, num_years=5)
    away_hist = get_team_multi_season_stats(away_id, league_id, num_years=5)
    if home_hist:
        lines.append(f"{teams['home']['name']} son sezonlar: " + "; ".join(
            f"{h['season']}: {h['wins']}G{h['draws']}B{h['loses']}M, gol {h['goals_for']}-{h['goals_against']}"
            for h in home_hist
        ))
    if away_hist:
        lines.append(f"{teams['away']['name']} son sezonlar: " + "; ".join(
            f"{h['season']}: {h['wins']}G{h['draws']}B{h['loses']}M, gol {h['goals_for']}-{h['goals_against']}"
            for h in away_hist
        ))
...
async def show_history(update, context, fixture_id):
    query = update.callback_query
    await query.edit_message_text("⏳ Son 5 sezonluk veriler çekiliyor (biraz sürebilir)...")
    fixture = context.chat_data.get("fixtures_cache", {}).get(fixture_id) or get_fixture_details(fixture_id)
    if not fixture:
        await query.edit_message_text("Maç bulunamadı.", reply_markup=match_detail_menu(fixture_id))
        return
    ...
    home_hist = get_team_multi_season_stats(home_id, league_id)
    away_hist = get_team_multi_season_stats(away_id, league_id)

    def _format(team_name, hist):
        if not hist:
            return f"*{team_name}*\nBu lig için geçmiş sezon verisi bulunamadı."
        lines = [f"*{team_name}*"]
        for h in hist:
            lines.append(
                f"  {h['season']}: {h['played']} maç — {h['wins']}G {h['draws']}B {h['loses']}M "
                f"| Gol: {h['goals_for']}-{h['goals_against']}"
            )
        return "\n".join(lines)

    text = (
        f"📅 *Son 5 Sezon — {teams['home']['name']} vs {teams['away']['name']}*\n\n"
        f"{_format(teams['home']['name'], home_hist)}\n\n"
        f"{_format(teams['away']['name'], away_hist)}\n\n"
        "_Not: Veriler sadece bu ligdeki performansı gösterir, takımların o sezon başka ligde/kupada "
        "oynadığı maçları içermez._"
    )
    await query.edit_message_text(text[:4000], reply_markup=match_detail_menu(fixture_id), parse_mode="Markdown")
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu:teamstats":
        await prompt_team_stats(update, context); return
    ...
    if data.startswith("match:"):
        ...
        if action is None:
            ...
            await show_history(update, context, fixture_id)
        elif action == "ai":
...
async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif mode == "awaiting_team_stats":
        await handle_team_stats_text(update, context)
